...
var gm = require('gm');
//settings
var db = mongoose.createConnection('localhost','evo');
...
function resizeImages(images, dimension){
...
var cms = module.exports = {
			...
			saveFile(req,null,function(err,fields,files){
				...
				for(var i in data){
					if(data[i] in files === true){
						var crop_files = JSON.parse(fields[i+"-crop"]);
						var _files = [];
						...
						var type = getMongooseFieldType(settings[feature].local[i].type);
						if(type === 'array'){
							data[i] = _files
						}else{
							data[i] = _files[0]
	...
	pageStructure:function generateMainPages(){
		...
		var obj = {};
		_.keys(settings).forEach(function(feature){
